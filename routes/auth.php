...
Route::middleware('guest')->group(function (Router $router) {
    $router->controller(AuthenticatedSessionController::class)->group(function (Router $router) {
        $router->get('/login', 'create')->name('login');
        $router->post('/login', 'store');
    });

    $router->name('password.')->group(function (Router $router) {
        $router->controller(PasswordResetLinkController::class)->group(function (Router $router) {
            $router->get('/forgot-password', 'create')->name('request');
            $router->post('/forgot-password', 'store')->name('email');
        });

        $router->controller(NewPasswordController::class)->group(function (Router $router) {
...
Route::middleware('auth')->group(function (Router $router) {
    $router->name('verification.')->group(function (Router $router) {
        ...
        $router->middleware('throttle:6,1')->group(function (Router $router) {
    ...
    $router->controller(ConfirmablePasswordController::class)
        ->name('password.')->group(function (Router $router) {
